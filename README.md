...
> **Why not just cut at keyframes?** You can stream-copy losslessly only at I-frames — but the cut points come from motion, not the GOP grid, so they almost never land on one. Snapping each cut to the nearest keyframe would be up to a full GOP (~1 s here) off, leaving pauses in or clipping motion. Re-encoding the sub-GOP sliver before each keyframe is what buys frame-accurate cuts *and* a mostly-lossless copy (~90% of output frames stay bit-identical); the alternatives are coarse keyframe-only cuts or re-encoding the whole thing (`--mode reencode`).
